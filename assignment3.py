...
class Assignment3:
    ...
    def areabetween(self, f1: callable, f2: callable) -> np.float32:
        ...
        """
        f = lambda a: abs(f2(a) - f1(a))
        x = assignment2.Assignment2().intersections(f1, f2, 1, 100)
        if not x or len(np.asarray(x)) < 2:
            return np.NaN
        x = sorted(x)
        curr_area = 0
        for i in range(len(x) - 1):
            curr_area += self.integrate(f, x[i], x[i + 1], 100)

        return np.float32(abs(curr_area))
